		...
		<div class="container">
          ...
          <div class="col-lg-8 ftco-animate">
            <?php if( ! empty($category_news))
              {
                foreach($category_news as $cn => $nc){?>
                <div class="block-21 mb-4 d-flex">
                    
                    <div class="text">
                        <h2 class="mb-3"><a href="<?php echo base_url('news/show_news/'.$nc->id);?>"><?php echo $nc->title;?></a></h2>
                        <?php echo $nc->description;?>
                        <div class="meta">
                            <div><a href="<?php echo base_url('news/show_news/'.$nc->id);?>"><span class="icon-calendar"></span> <?php echo date('M',strtotime($nc->date)); ?>. <?php echo date('d',strtotime($nc->date)); ?>, <?php echo date('Y',strtotime($nc->date)); ?></a></div>
                            <div><a href="<?php echo base_url('news/show_news/'.$nc->id);?>"><span class="icon-person"></span> <?php echo $nc->author;?></a></div>
                            <div><a href="<?php echo base_url('news/show_news/'.$nc->id);?>"><span class="icon-eye"></span> <?php echo $nc->count_read;?></a></div>
                        </div>
                    </div>
                </div>
                <?php }
